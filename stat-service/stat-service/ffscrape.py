...
# https://www.forexfactory.com/calendar?day=jan1.2023
    ...
    for yearly_data in data:
        for monthly_data in yearly_data:
            for indicator in monthly_data:
                ...
                cursor.execute("INSERT INTO INDICATOR_VALUES(IndicatorID,Actual,ReleaseDate) values(?,?,?)", id[0],
                               indicator.actual,
                               indicator.date)
                cnxn.commit()


def get_daily_data(d, dt):
    scraper = cloudscraper.create_scraper(delay=10, browser="chrome")
    url = "https://www.forexfactory.com/calendar?day=" + d
    html = scraper.get(url).text
...
def insert_latest_data():
    # Query DB for max date
    cnxn = pyodbc.connect(connectionString)
    cursor = cnxn.cursor()
    cursor.execute("SELECT MAX(ReleaseDate) FROM INDICATOR_VALUES;")
    q_resp = cursor.fetchone()
    recent_date = q_resp[0]
    new_date = datetime.datetime.today().date()

    # Build indicator dict
    event_dict = {}
    cursor.execute("SELECT EventName, ID FROM INDICATORS")
    event_info = cursor.fetchone()
    while event_info is not None:
        event_dict[event_info[0]] = event_info[1]
        event_info = cursor.fetchone()

    # Insert daily data until we reach the max(date) in db
    while new_date != recent_date:
        time.sleep(1)
        print(new_date)
        daily_data = get_daily_data(convertDate(new_date), new_date)
        for i in daily_data:
            if i.name in event_dict:
                cursor.execute("INSERT INTO INDICATOR_VALUES(IndicatorID,Actual,ReleaseDate) values(?,?,?)",
                               event_dict[i.name], i.actual, i.date)
        cursor.commit()
